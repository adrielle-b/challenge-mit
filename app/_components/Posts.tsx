...
import { requestCreatePost, requestGetPosts,
...
export default function Posts() {
  ...
  const saveEditedPost = async (id: string, postEdit: bodyUpdatePost) => {
    try {
      const token = localStorage.getItem('token');
      setToken(token);
    ...
    <main className="bg-black text-gray-300 p-8">
      <section className="max-w-4xl mx-auto">
        ...
        <input
          type="text"
          name="title"
          className="w-full bg-gray-700 bg-opacity-50 text-white placeholder-gray-400
